...
public class SeleniumVendorTest extends BaseTest {
    ...
    @Test(description = "Тест с корректной суммой и некорректным производителем")
    ...
    public void step3() throws IOException {
        ...
        By bbk = By.xpath("//div[@data-zone-name='search-filters-aside']//input[@type='checkbox' and @name='Производитель BBK']");
        By[] weBy = {bbk};
        searchPage.fillVendor(weBy);

        String price = "20000";
        searchPage.setPriceTo(price);
    ...
    private void step4() throws IOException {
        ...
        takeScreenshot(driver);
        if (!productPage.getWeArticleVendor().getText().contains("LG") && !productPage.getWeArticleVendor().getText().contains("Samsung"))
            softAssertions.fail("Производитель не Samsung/LG");
